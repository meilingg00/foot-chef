...
const inputRef = db.ref("smartFloor/input");
...
// Listen for input from the Smart Floor
inputRef.on("value", snap => {
    const cellIndex = snap.val(); // Assuming this returns 0-8
    
    // Validate input
    if (cellIndex !== null && cellIndex >= 0 && cellIndex < 9) {
        const targetCell = document.querySelector(`.cell[data-index="${cellIndex}"]`);
        if (targetCell) {
            // Trigger the click logic programmatically
            targetCell.click();
        }
    }
});
...
function showEndScreen() {
    ...
    const box = document.getElementById("finalDishList");
    box.innerHTML = "";
